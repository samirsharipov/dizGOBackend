...
public class ProductAboutRepository2 {
    ...
    public List<ProductAbout2> getAllDescriptionsWithKeyword(String keyword) {
        String sql = "SELECT pa.id, pa.created_at, pa.update_at, pa.amount, pa.description,\n" +
                ...
                "LEFT JOIN product p ON pa.product_id = p.id\n" +
                "LEFT JOIN product_type_price ptp ON pa.product_type_price_id = ptp.id\n" +
                "LEFT JOIN measurement m ON p.measurement_id = m.id\n" + // Tegishli qo'shilgan qism
                "WHERE pa.description LIKE ? AND pa.description = 'ISHLAB CHIQARILDI';"; // Tegishli qo'shilgan qism

        return jdbcTemplate.query(sql, new Object[]{"%" + keyword + "%"}, (resultSet, rowNum) -> {
            ProductAbout2 productAbout2 = new ProductAbout2();
    ...
    public List<ProductAbout2> getAllDescriptionsHomashyo(String keyword) {
        String sql = "SELECT pa.id, pa.created_at, pa.update_at, pa.amount, pa.description,\n" +
                ...
                "LEFT JOIN product_type_price ptp ON pa.product_type_price_id = ptp.id\n" +
                "LEFT JOIN measurement m ON p.measurement_id = m.id\n" + // Tegishli qo'shilgan qism
                "WHERE pa.description LIKE ? AND pa.description = 'XOMASHYO';"; // Tegishli qo'shilgan qism
    ...
    public List<ProductAbout2> getAllDescriptionsBrak(String keyword) {
        String sql = "SELECT pa.id, pa.created_at, pa.update_at, pa.amount, pa.description,\n" +
                ...
                "LEFT JOIN product_type_price ptp ON pa.product_type_price_id = ptp.id\n" +
                "LEFT JOIN measurement m ON p.measurement_id = m.id\n" + // Tegishli qo'shilgan qism
                "WHERE pa.description LIKE ? AND pa.description = 'BRAKDAN CHIQDI';";
