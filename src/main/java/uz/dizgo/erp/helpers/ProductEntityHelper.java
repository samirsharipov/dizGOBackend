...
public class ProductEntityHelper {
    ...
    public Product cloneProduct(UUID productId, Branch branch) {
        Product originalProduct = productRepository.findById(productId).orElseThrow(() -> new IllegalArgumentException("Product not found"));

        Business business = branch.getBusiness();

        Category category = null;
        if (originalProduct.getCategory() != null) {
            category = findOrCreateEntity(() -> categoryRepository.findByBusiness_IdAndName(business.getId(), originalProduct.getCategory().getName()), () -> createCategory(originalProduct.getCategory(), business), categoryRepository::save);
        }

        Brand brand = null;
        if (originalProduct.getBrand() != null) {
            brand = findOrCreateEntity(() -> brandRepository.findByBusiness_IdAndName(business.getId(), originalProduct.getBrand().getName()), () -> createBrand(originalProduct.getBrand(), business), brandRepository::save);
        }

        Measurement measurement = null;
        if (originalProduct.getMeasurement() != null) {
            measurement = findOrCreateEntity(() -> measurementRepository.findByBusinessIdAndName(business.getId(), originalProduct.getMeasurement().getName()), () -> createMeasurement(originalProduct.getMeasurement(), business), measurementRepository::save);
        }
        ...
        Optional.ofNullable(originalProduct.getTranslations()).ifPresent(translations -> translations.forEach(translation -> {
    ...
    private void cloneProductFields(Product originalProduct, Product clonedProduct, Branch branch, Business business, Category category, Brand brand, Measurement measurement) {
...
//        if (originalProduct.getPhoto() != null) {
//            Optional<Attachment> optionalAttachment = attachmentRepository.findById(originalProduct.getPhoto().getId());
//            optionalAttachment.ifPresent(clonedProduct::setPhoto);
//
//        }
