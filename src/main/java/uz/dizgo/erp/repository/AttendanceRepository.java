...
import uz.dizgo.erp.payload.statistics.AttendanceStat;
...
""", nativeQuery = true)
    ...
    @Query("SELECT new uz.dizgo.erp.payload.EmployeeWorkDurationDto(a.id,e.id, CONCAT(e.firstName, ' ', e.lastName), SUM(a.workDuration)) " +
            "FROM Attendance a " +
